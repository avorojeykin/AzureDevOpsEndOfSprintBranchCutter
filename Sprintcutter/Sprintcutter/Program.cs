...
namespace AzureSprintcutter
{
    internal class Program
    {
        ...
        private static string _deleteBranch = "https://dev.azure.com/PlaceHolder/_apis/git/repositories/{0}/refs?api-version=6.0";
        ...
        private static bool deleteCustomBranches()
        {
            ...
            Guid projectId;
            List<GitBranchStats> branches = new List<GitBranchStats>();
            List<GitBranchStats> vetbranches = new List<GitBranchStats>();
            VssBasicCredential credentials = new VssBasicCredential(string.Empty, _pat);
            VssConnection connection = new VssConnection(new Uri(string.Format("https://dev.azure.com/{0}", "PlaceHolder")), credentials);
            using (GitHttpClient gitClient = connection.GetClient<GitHttpClient>())
            {
                GitRepository repo = gitClient.GetRepositoryAsync("PlaceHolder", "CustomRepo").Result;
                branches = gitClient.GetBranchesAsync(repo.Id).SyncResult();

                foreach (GitBranchStats branch in branches)
                {
                    if (branch.Name.Contains("CustomFeatureBranch"))
                    {
                        vetbranches.Add(branch);
                    }
                }

                foreach (GitBranchStats branch in vetbranches)
                {
                    var gitRef = gitClient.UpdateRefsAsync(new GitRefUpdate[] {new GitRefUpdate
                            {
                                Name = $"refs/heads/{branch.Name}",
        ...
        private static bool createTag(string branchName, string tagName)
        {
            ...
            using (GitHttpClient gitClient = connection.GetClient<GitHttpClient>())
            {
                ...
                {
                    var createdTag = gitClient.CreateAnnotatedTagAsync(newTag, "PlaceHolder", repo.Id).Result;
                    return true;
                }
                else
                {
        ...
        static void Main(string[] args)
        {
            ...
            if (_repoArray != null)
            {
                foreach (var repo in _repoArray)
                {
                    ...
                    // Deleting old Release Candidate Branches
                    bool releaseCandidatesDeleted = deletePreviousReleaseBranches();
                    if (releaseCandidatesDeleted)
                    {
                        Console.WriteLine("Previous Release Candidate Branches Deleted");
                    }
                    else
                    {
                        Console.WriteLine("No Release Candidate Branches to Delete");
                    }
                }
                ...
                if (branchDeleted)
                {
                    Console.WriteLine("Vet Feature Branches Deleted");
                }
                else
                {
                    Console.WriteLine("No Vet Feature Branches to Delete");
                }
            }
            else
            {
